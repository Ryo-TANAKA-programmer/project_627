...
cv.glm(diabetes_1, reduced_dia_reg)$delta
```
...
training <- sample(1:nrow(diabetes), 
                   0.6*nrow(diabetes))
...
set.seed(1234)
DIA_training <- diabetes[training,-ycol]
...
sum(MV_tree_classifications == MV_test_results) / nrow(DC[-training_mv,])
...
for (i in seq(from = 0.0005, 
              to = 0.05, 
              by = 0.0005)) {
  DC.tree <- tree(DEDUCTIBLE ~ ., data=DC[training,], mindev=i)
  DC.tree.proportions <- predict(DC.tree,DC[-training,])
  DC.tree.classifications <- round(DC.tree.proportions,0)
  error.rate <- 1- (sum(DC.tree.classifications == DC.test.results) / nrow(DC[-training,]))
...
DC.best.tree <- tree(DEDUCTIBLE ~ ., data=DC[training,], mindev=best.mindev)
plot(DC.best.tree)
text(DC.best.tree, cex=0.5)
```
