...
diabetes_tree <- diabetes[-c(11,12)]
...
DIA_BEST_TREE <- tree(CLASS_BINARY ~ ., 
                      data = diabetes_tree[training,], 
                      mindev = best.mindev)
plot(DIA_BEST_TREE)
...
```
## Ryo is working on LOOCV using Beau's

```{r}
# I'm still editing this code
full_reg_BINARY_LOOCV <- glm(CLASS_BINARY ~ Gender + AGE + Urea + Cr + HbA1c + Chol + TG + 
                         HDL + LDL + VLDL + BMI_Category,
                       family = "binomial",
                       data = diabetes_LOOCV)   # initial model

cv_result <- cv.glm(diabetes, 
                    full_reg_BINARY, 
                    K = nrow(diabetes))

test.mse.loo <- cv_result$delta[1]
test.rmse.loo <- sqrt(test.mse.loo)
test.error.loocv <- cbind("MSE" = test.mse.loo, "RMSE" = test.rmse.loo)
test.error.loocv

test.mse.loo <- cv.glm(diabetes_LOOCV, 
                       full_reg_BINARY_LOOCV)$delta[1] 
