...
## Beau: K-fold CV using Logistic Regression

```{r}

full_reg_BINARY_kfold <- glm(CLASS_BINARY ~ Gender + AGE + Urea + Cr + HbA1c + Chol +
                               TG + HDL + LDL + VLDL + BMI,
                             family = "binomial",
                             data = diabetes)   # initial model

cv_loocv <- cv.glm(diabetes, full_reg_BINARY_kfold, K = 10)

kfold_error <- cv_loocv$delta[1]

# Print the K-fold prediction error
print(paste("K-fold Prediction Error:", kfold_error))
```
...
full_reg_BINARY_kfold_train <- glm(CLASS_BINARY ~ Gender + AGE + Urea + Cr + HbA1c + Chol + TG + 
                                     HDL + LDL + VLDL + BMI,
                                   family = "binomial",
                                   data = training_data)   # initial model

# Predict probabilities on the test data
Prob <- predict(full_reg_BINARY_kfold_train, newdata = test_data, type = "response")
...
X_train <- model.matrix(CLASS_BINARY ~ Gender + AGE + Urea + Cr + HbA1c + Chol + TG +HDL + LDL + VLDL + BMI , data = train_data)[,-1]
Y_train <- train_data$CLASS_BINARY
X_test <- model.matrix(CLASS_BINARY ~ Gender + AGE + Urea + Cr + HbA1c + Chol + TG + HDL + LDL + VLDL + BMI , data = test_data)[,-1]
...
```{r}
formula <- CLASS_BINARY ~ Gender + AGE + Urea + Cr + HbA1c + Chol + TG +HDL + LDL + VLDL + BMI 
...
#glm(formula = CLASS_BINARY ~ Gender + HbA1c + Chol + TG + BMI, 
   # family = "binomial", data = diabetes) best model with lowest AIC
```

## Beau: K-fold CV using stepwise Logistic Regression

```{r}

stepwise_reg_BINARY_kfold <- glm(CLASS_BINARY ~ Gender + HbA1c + Chol + TG + BMI,
                             family = "binomial",
                             data = diabetes)   # initial model

cv_loocv <- cv.glm(diabetes, stepwise_reg_BINARY_kfold, K = 10)

stepwise_kfold_error <- cv_loocv$delta[1]

# Print the K-fold prediction error
print(paste("K-fold Prediction Error:", stepwise_kfold_error))
```

## Beau: Optimal threshold for stepwise logistic regression
...
stepwise_reg_BINARY_kfold_train<- glm(CLASS_BINARY ~ Gender + HbA1c + Chol + TG + BMI,
                             family = "binomial",
                             data = training_data)  


# Predict probabilities on the test data
Prob <- predict(stepwise_reg_BINARY_kfold_train, newdata = test_data, type = "response")
